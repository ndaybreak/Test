...
ColReorder.prototype = {
	...
	"_fnConstruct": function ()
	{
		...
		for ( i=0, iLen=this.s.dt.aoColumns.length ; i<iLen ; i++ )
		{
			if ( i > this.s.fixed-1 )
			{
				...
				// this._fnMouseListener( i, this.s.dt.aoColumns[i].nTh.children[0] );
			}
	...
	"_fnMouseListener": function ( i, nTh )
	{
		...
		if(nTh.children.length>1){
			bindBlock = nTh.children[0];
		}

		//Martin Marchetta (rebind events since after column re-order they use wrong column indices)
		$(bindBlock).unbind('mousemove.ColReorder');
		$(bindBlock).unbind('mousedown.ColReorder');
		////////////////

    // listen to mousemove event for resize
    if (this.s.allowResize) {
  		$(bindBlock).bind( 'mousemove.ColReorder', function (e) {   
    ...
    // listen to mousedown event
		$(bindBlock).bind( 'mousedown.ColReorder', function (e) {
